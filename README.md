...
```c
enum имя_перечисления { ИМЕНА_МЕТОК, ЧЕРЕЗ_ЗАПЯТУЮ }; // точка с запятой в конце
```
...
```
После этого в нашем языке появляется новый тип `color_t`, и мы можем использовать этот тип для переменных.
...
struct point_t p4 = {.y = 5.0};       // частичная инициализация, x будет 0
```

Именованная инициализация (designated initializers) позволяет указывать значения полей в любом порядке 
и инициализировать только нужные поля, остальные будут заполнены нулями.
...
Или можно объявить `typedef` отдельно:
...
Программа на языке си записывается в файлы. В большинстве случаев эти файлы имеют расширение `.c`.
...
Утверждение-выражение (expression statement) -- это выражение, за которым идёт точка с запятой.
